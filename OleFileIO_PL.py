...
class _OleStream(StringIO.StringIO):
    ...
    def __init__(self, fp, sect, size, offset, sectorsize, fat):
        ...
        for i in xrange(nb_sectors):
            debug("i = %d, sect = %#x (%d)" % (i, sect, sect))
            # Sector index may be ENDOFCHAIN, but only if size was unknown
            if sect == ENDOFCHAIN:
                if unknown_size:
                    break
                elif i == 1:
                    #apparently file CVE-2009-3129.xls contains a workbook stream
                    #where the first sect is set to ENDOFCHAIN, even though there
                    #is more data to follow
                    print "WARNING grep me"
                    sect = 1
                else:
...
class OleFileIO:
    ...
    def open(self, filename):
        ...
        # SL: saved cslid for write support
